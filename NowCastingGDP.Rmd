...
summary(month0)
#predict(month0$model, n.ahead=1, newregs = predict(X2$M0)$fitted)
forecast::forecast(month0, xreg = X2$M0)$mean
```
...
summary(month01)
predict(month01$model, n.ahead=1, newregs = cbind(predict(X2$M0)$fitted,predict(X2$M1)$fitted))
...
summary(month012)
predict(month012$model, n.ahead=1,cbind(predict(X2$M0)$fitted,predict(X2$M1)$fitted,predict(X2$M2)$fitted))
...
```
```{r}
QdiffTest2 = cbind(predict(X2$M1)$fitted,predict(X2$M2)$fitted)
valid = predict(model.5, newxreg=QdiffTest2)
valid$pred
```
