...
model.4 <- arima( x = Qdiff, order=c(1,1,1),  xreg = diff(log(X2[,c("M0","M1","M2")]), differences = 1))
...
LFS_2018 <- LFS_2018 %>% mutate(survdate = ymd(survdate))
...
```{r}
QdiffTest = cbind(X2$M0,X2$M1,X2$M2)
test_model2 = auto.arima(Qdiff, xreg=diff(log(QdiffTest)))
forecast::forecast(test_model2, lead=1, id = NULL, alpha = 0.05, output= TRUE)
```
