...
```
We test with AR 1 or 2 and MA 1 for arima model
...
Retrieve residuals from ARMA models
```{r}
Resid.1 <- model.1$residuals
Resid.2 <- model.2$residuals
Resid.3 <- model.3$residuals
Resid.4 <- model.4$residuals
```
...
adf.test(X2$Q, nlag=NULL, output = TRUE)
adf.test(X2$M0, nlag=12, output = TRUE)
adf.test(X2$M1, nlag=12, output = TRUE)
adf.test(X2$M2, nlag=12, output = TRUE)
```
...
wages <- wages %>% filter(`Seasonal adjustment` == "Seasonally adjusted") %>% filter(GEO=="Canada") %>% filter(Sector == "Wages and salaries")

wages
```

```{r}
LFS_2018 <- LFS_2018 %>% unite(survdate, survyear, survmnth, sep = "-") %>% mutate(survdate = paste(survdate, "1", sep = "-"))

LFS_2018 %>% mutate(survdate = ymd(survdate))
```
...
```{r}
## Now to turn LFS into a TS

# Q.1 = LFS_2018 %>%  
#   # find year & quarter
#   mutate( Y = year( survdate ), Q = quarter( survdate ),
#           index = yearqtr( Y + Q/4 ) ) %>%  
#   xts( x=.$lfsstat, order.by =.$index) 
# # plot(Q)
# # Monthly data 
# M.1 = LFS_2018 %>% 
#   # find year, quarter, month, and month-in-quarter
...
#           index = yearqtr( Y + Q/4 ), 
#           M = month( survdate ),
#           MinQ = paste( "M", M%%13, sep="" ) ) %>%  
#   # spread monthly data into 12 columns one for each month-in-quarter
#   pivot_wider(id_cols = index, names_from = MinQ, 
#               values_from = lfsstat ) %>% 
#   xts( x=.[,c("M0","M1","M2","M3", "M4", "M5", "M6", "M7", "M8", "M9", "M10", "M11")], order.by =.$index)
# 
# LFS2 = merge(Q.1, M.1, join = "inner" )
```
...
ts(CanWS, start = c(1997,04), end = c(2019,6), frequency = 12) -> CanWS_ts_Monthly
