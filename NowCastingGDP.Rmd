...
#GO OVER THIS WITH SOTIRIOS 
for(i in (n - nCV:1) ){
  # Increasing subsets of y,x
  yt = QScale$Q[1:i]; xt = QScale[1:i, c("M0")]  
  ...
  y_fe[i+1] = predict( out, newxreg = xt[i+1] )$pred
  ...
  y_fe.[i+1] = b0 + b1 * xt[i+1] + phi * ( y[i] - b0 - b1 * xt[i] )
}
...
models_list <- list(
    Arima = list(
        y = train_ts[,1],order = c(1,1,1), xreg = train_ts[,2], include.drift = TRUE  
        ),
    Arima = list(
        y = train_ts[,1],order = c(1,1,1), xreg = train_ts[,2:3], include.drift = TRUE
    ),
    Arima = list(
        y = train_ts[,1],order = c(1,1,1), xreg = train_ts[,2:4], include.drift = TRUE
    )
)
