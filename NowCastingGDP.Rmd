...
for(i in (n - nCV:1) ){
  ...
  out2 = forecast::Arima( yt , order=c(1,1,1),  xreg = matrix( X[1:i,c("M0","M1")] ), include.drift = T)
  fit = forecast::forecast(out2, xreg = matrix( X[i+1, c("M0","M1")], 1 )  )
...
for(i in (n - nCV:1) ){
  ...
  out2 = forecast::Arima( yt , order=c(1,1,1),  xreg = matrix( X[1:i,c("M0","M1","M2")] ), include.drift = T)
  fit = forecast::forecast(out2, xreg = matrix( X[i+1, c("M0","M1","M2")], 1 )  )
